...
FIGURES = [
    (
        "fig1",
        "图 1",
        "IMA-AP 泄漏代理随缝线缩短变化：Galili 峰缩期映射 vs 临床 η 映射",
...
""",
    ...
    (
        "fig4",
        "图 4",
        "IMA-CS 临床映射：泄漏代理–筛查 Pareto（CS–LCx 与 NiTi 交变应变）",
...
""",
    ...
    (
        "fig5",
        "图 5",
        "双缝线交界因子敏感性（0.25/0.5/0.75/1.0）——假设参数，非发现声称",
...
def load_artifacts():
    ...
    tables = {
        ...
        "pareto": (
            read_csv(TABLE_DIR / "pareto_regurg_vs_risk_screens.csv")
            if (TABLE_DIR / "pareto_regurg_vs_risk_screens.csv").is_file()
            else read_csv(TABLE_DIR / "pareto_regurg_vs_safety.csv")
            if (TABLE_DIR / "pareto_regurg_vs_safety.csv").is_file()
            else []
        ),
...
| 假设下最优候选（seed=42） | {rec_rec['device']} {rec_rec['shortening_pct']}%（n_sutures={rec_rec['n_sutures']}）；AP↓ {rec_rec['ap_reduction_pct']:.1f}%；leakage_proxy≈{rec_rec['physics_regurgitation_pct']:.4f}%；jet=`{rec_rec['jet_location']}`；response_path=`{rec.get('response_path','fitted_response')}` |
...
**结果（seed=42）：** 先报告诚实留出/交叉验证：历史 pre-dampen 规则基 baseline MAE ROA≈50.18 mm²、泄漏≈1.445 pp；当前 dampened 规则基留出 MAE 见下文 CV 节；折内响应模型留出子集 MAE ROA≈6.37 mm²、泄漏≈0.245 pp（AP70≈6.60 mm² / 0.198 pp）。**随后**才是假设驱动的探索性排序：n_total_points={n_tot}，n_device_candidates={n_dev}，n_feasible_device_candidates={n_feas}，p_feasible_device_candidates≈{float(p_feas):.4f}；`fitted_response` 下最优候选为 **{rec_rec['device']} {rec_rec['shortening_pct']}%**（AP↓{rec_rec['ap_reduction_pct']:.1f}%，leakage_proxy≈{rec_rec['physics_regurgitation_pct']:.4f}%）。双缝线仅为图 5 假设敏感性，不是默认第一名。
...
Results (seed=42): Held-out / fold-wise CV first (historical rule-based baseline MAE ROA 50.18 / 1.445; fold-wise held-out subset ≈6.37 / 0.245; AP70 ≈6.60 / 0.198). Then assumption-driven ranking: {n_dev} device candidates / {n_feas} feasible (p≈{float(p_feas):.4f}); best under `fitted_response` is {rec_rec['device']} {rec_rec['shortening_pct']}% (AP↓{rec_rec['ap_reduction_pct']:.1f}%, leak≈{rec_rec['physics_regurgitation_pct']:.4f}%). Dual suture is hypothesis sensitivity (Fig. 5), not the default top rank.
...
**备选最佳 IMA-AP 单缝线：** {alt_s['shortening_pct']}%；AP↓{alt_s['ap_reduction_pct']:.1f}%；leakage_proxy≈{alt_s['physics_regurgitation_pct']:.4f}%；jet=`{alt_s['jet_location']}`；交界分数 {alt_s['commissural_fraction']:.3f}。

**备选最佳 IMA-CS（常与最优重合）：** {alt_cs['shortening_pct']}%；AP↓{alt_cs['ap_reduction_pct']:.1f}%；leakage_proxy≈{alt_cs['physics_regurgitation_pct']:.4f}%；CS–LCx={alt_cs['cs_lcx_mm']} mm；NiTi={alt_cs['niti_alternating_strain_pct']}%。
...
在 seed-42、`fitted_response`、假设映射与默认约束下，一层代理情景排序给出可复现**假设下最优候选**：**{rec_rec['device']} {rec_rec['shortening_pct']}%**，AP 缩减 **{rec_rec['ap_reduction_pct']:.1f}%**，leakage_proxy **{rec_rec['physics_regurgitation_pct']:.4f}%**，jet=`{rec_rec['jet_location']}`。该结果展示了如何把 AP 窗口、射流机制与 LCx/NiTi **筛查**写入可复现研究原型，同时严格保持 Level-1 边界。
...
<div class="wrap">
  <header class="cover" id="cover">
    ...
    <p class="meta">生成日期：{today} · 数据：<code>run_pipeline.py --seed 42 --paper</code> · 先 CV/留出再排序 · n_device={n_dev} 可行={n_feas} P≈{p_feas:.2f} · 最优 {rec_rec['device']} {rec_rec['shortening_pct']}% · leakage_proxy {phys:.4f}% · LHS top-1 {stab_txt}</p>
  </header>
  ...
  <section id="abstract">
    ...
    <p><strong>结果（seed=42）：</strong>先留出/CV（历史规则基 baseline MAE ROA≈50.18 / leak≈1.445；折内留出子集≈6.37 / 0.245；AP70≈6.60 mm² / 0.198 pp）。再排序：n_total={n_tot}，n_device={n_dev}，可行={n_feas}，p≈{p_feas:.4f}；`fitted_response` 最优为 <strong>{rec_rec['device']} {rec_rec['shortening_pct']}%</strong>（AP↓{rec_rec['ap_reduction_pct']:.1f}%，leak≈{phys:.4f}%）。双缝线仅图 5 敏感性。</p>
    <p><strong>结论：</strong>支持探索性筛查与诚实留出报告；非临床推荐、非患者级外部验证。</p>
    <h3>English abstract</h3>
    <p>Held-out / fold-wise CV first, then assumption-driven ranking (n_device={n_dev}, feasible={n_feas}, p≈{p_feas:.4f}). Best under fitted_response: {rec_rec['device']} {rec_rec['shortening_pct']}% (AP↓{rec_rec['ap_reduction_pct']:.1f}%, leak≈{phys:.4f}%). Dual suture is Fig. 5 sensitivity only. Not validated prediction.</p>
  ...
  <section id="methods">
    ...
    <p><strong>几何与病理。</strong>基线舒张期瓣环周长约 118.5 mm、AP 34.4 mm；峰缩期疾病 AP 约 26.1 mm（Galili 表）。后乳头肌病理以被动单元分数进入代数力学代理。AP 为文献几何输入（<code>ap_role=literature_mapping_input</code>），不作预测 MAE。</p>
    <p><strong>装置运动学。</strong>Galili 映射插值发表峰缩期 AP；临床/规划映射 \\(AP\\_reduction\\%=\\eta\\times shortening\\%\\)（η_AP=0.30，η_CS=0.55；均为假设先验，η_CS 非 MAVERIC/ARTO 拟合）。AP 缩减上限 20%。LHS（N=120）扰动 η、双缝交界因子、CS–LCx 基线与斜率。</p>
    <p><strong>力学与泄漏代理。</strong>代数对合间隙/应变/接触代理 → ROA 与 jet 标签；文献校准泄漏代理输出 <code>leakage_proxy_pct</code>（别名 physics_regurgitation_pct）。Physics % ≠ 临床反流容积。Dryad <code>contact_fraction</code> 进入 fitted 响应特征；标量 AP/ROA/leak 仍以表为准。</p>
    <p><strong>响应路径。</strong><code>response_path</code>∈{{fitted_response, rule_based_proxy, hybrid_ap_extreme}}；论文 seed-42 默认 <code>fitted_response</code>。真折内 CV：<code>analysis/fit_response_model.py</code>；规则基 blend-off 诊断：<code>tools/loo_evaluate.py</code>（二者不可混称）。</p>
    <p><strong>情景排序。</strong>最小化泄漏代理；约束含 CS–LCx≥8.6 mm（文献风险筛查；默认基线 11.0 mm）与 NiTi 交变应变&lt;0.4%。网格：IMA-AP 10–70%（步长 5%）；IMA-CS 10–25%（步长 2%）；双缝线同 AP 网格。Pareto 分全局/CS/AP 族，禁止用 1e9/0 填补缺失 LCx/NiTi。</p>
    <p>论文图由 SciencePlots + Times New Roman（或 Liberation/STIX 回退）、dpi≥300 导出；轴标签英文，中文释义见长图注。</p>
  </section>
...
{held_block}
  ...
  <section id="discussion">
    ...
    <ul class="compact">
      ...
      <li>双缝线交界因子为假设参数（图 5）；默认排序第一名是 IMA-CS 而非双缝线。</li>
      <li>η±20% 在当前路径下仍保持 IMA-CS 20%，但 AP↓ 变为 8.8/11.0/13.2%；LHS top-1≈{stab_txt}。</li>
    </ul>
  </section>

  <section id="conclusions">
    <h2 class="sec">七、结论</h2>
    <p>在 seed-42 与默认约束下，Layer-1 情景排序<strong>假设下最优候选</strong> <strong>{rec_rec['device']} {rec_rec['shortening_pct']}%</strong>，AP 缩减 <strong>{rec_rec['ap_reduction_pct']:.1f}%</strong>，leakage_proxy <strong>{phys:.4f}%</strong>，jet=<code>{rec_rec['jet_location']}</code>。该输出是可追溯探索性筛查结果，不是治疗处方。</p>
  </section>
...
<p><strong>Baseline：</strong>seed-42（IMA-CS 20% / AP↓11% / leakage_proxy≈0.391% / central / fitted_response）、SciencePlots 五图、golden tests、成熟化后的 manuscript / report / paper。</p>
...
def main() -> None:
    ...
    test_status = (
        "PASS — PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 pytest tests/ -q; "
        "PASS — python run_pipeline.py --seed 42 --paper --no-export "
        "(planner: IMA-CS 20%, AP↓11.0%, leakage_proxy≈0.391%, jet=central, response_path=fitted_response)"
    )
    pdf_status = "生成中"
