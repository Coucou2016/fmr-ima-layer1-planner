"""Design sweep, jet classifier, clinical mapping, and scenario ranker."""
...
def test_clinical_references_yaml_loads():
    ...
    assert "ARTO" in data["maveric"]["device"] or data["maveric"]["mechanism_class"] == "IMA-AP"
    assert data["carillon"]["mechanism_class"] == "IMA-CS"
    assert data["lcx_compression"]["cs_lcx_threshold_mm"] == 8.6
    assert data["galili_rsos_2022"]["regurgitation_pct_anchors"]["pathology"] == 5.26
    assert data["galili_rsos_2022"]["peak_systole"]["ap_diameter_mm"]["ima_ap_50"] == 15.9
    assert data["niti_fatigue"]["alternating_strain_pct_max"] == 0.4
    table = data["mapping_assumptions"]["ima_ap"]["galili_peak_systole_vs_planning"]
    row50 = next(r for r in table if r["suture_pct"] == 50)
    assert row50["galili_peak_sys_ap_mm"] == 15.9
    assert row50["planning_ap_red_pct_eta03"] == 15.0
...
def test_design_space_yaml_loads():
    ...
    assert len(ap) >= 12
    assert len(cs) >= 8
    assert abs(float(cfg["clinical_mapping"]["ap_transfer_eta_ima_cs"]) - 0.55) < 1e-9


def test_clinical_mapping_ap_reduction_planning_window():
    ...
    # Galili peak-systole facts
    g50 = IMA_AP(shortening_pct=50).apply()
    g70 = IMA_AP(shortening_pct=70).apply()
    assert abs(g50.ap_diameter_mm - 15.9) < 0.1
    assert abs(g70.ap_diameter_mm - 12.4) < 0.2


def test_ima_cs_clinical_eta_is_assumption_not_maveric_fit():
    """η_CS assumption prior; must NOT equal old MAVERIC 14.7% at 22% bridge."""
    dev = IMA_CS(bridge_shortening_pct=22, mapping_mode="clinical")
    # assumption η=0.55 → 12.1% AP, not 14.7% from deleted MAVERIC calibration
    assert abs(dev.ap_reduction_pct() - 12.1) < 0.15
    assert abs(dev.ap_reduction_pct() - 14.7) > 1.0
    galili = IMA_CS(bridge_shortening_pct=22).apply()
    assert abs(galili.ap_diameter_mm - 24.8) < 0.05
...
def test_sweep_galili_ap_nonmonotonic_and_cs_trend(tmp_path):
    ...
    assert abs(ap[50].ap_diameter_mm - 15.9) < 0.1
...
def test_evaluate_physics_not_blended_for_sweep_ids():
    ...
    assert abs(pt.ap_diameter_mm - 15.9) < 0.1
...
def test_paper_alignment_and_dual_tables():
    ...
    assert any("ARTO" in r["device_or_arm"] or "MAVERIC" in r["source"] for r in align)
    assert any("Carillon" in r["device_or_arm"] for r in align)
...
def test_clinical_references_reduce_fmr_directionality():
    ...
    assert data["maveric_alignment_policy"]["maveric_maps_to"] == "IMA-AP (ARTO)"
