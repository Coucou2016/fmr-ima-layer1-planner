...
def test_ima_cs_annulus_22pct():
    ...
    assert abs(g.ap_diameter_mm - 24.8) < 0.1


def test_ima_ap_ap_diameters_peak_systole():
...
def test_leakage_proxy_anchors():
    sph = SPHSurrogate(29000)
    g_path = HeartGeometry(ap_diameter_mm=26.1, cardiac_phase="peak_systole")
    elems = apply_papillary_pathology(make_papillary_mesh(200), posterior_fraction_passive=0.44)
    fea_path = run_mechanics_proxy("pathology", g_path, elems, None)
    ...
    fea_cs = run_mechanics_proxy("ima_cs_22", g_cs, elems, dev_cs)
    ...
    fea_ap = run_mechanics_proxy("ima_ap_50", g_ap, elems, dev_ap)
...
def test_ima_ap_70_worse_than_50():
    ...
    g70 = IMA_AP(shortening_pct=70).apply()
    elems = apply_papillary_pathology(make_papillary_mesh(200), posterior_fraction_passive=0.44)
    fea50 = run_mechanics_proxy("ima_ap_50", g50, elems, IMA_AP(50))
    fea70 = run_mechanics_proxy("ima_ap_70", g70, elems, IMA_AP(70))
...
def test_commissural_mechanism_not_arbitrary_multiplier():
    g_leak = HeartGeometry(ap_diameter_mm=12.4, cardiac_phase="peak_systole")
    g_ok = HeartGeometry(ap_diameter_mm=26.1, cardiac_phase="peak_systole")
...
def test_contacts_from_fea_reproducible():
    ...
    g = HeartGeometry(ap_diameter_mm=26.1, cardiac_phase="peak_systole")
    fea = run_mechanics_proxy("pathology", g, elems, None)
