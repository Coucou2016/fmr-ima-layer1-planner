...
def test_scenario_ranker_seed42_invariants(tmp_path):
    ...
    rec_a = run_scenario_ranker(
        seed=GOLDEN_SEED, mapping_mode="clinical", output_dir=tmp_path / "a", skip_uncertainty=True
    )

    rec_b = run_scenario_ranker(
        seed=GOLDEN_SEED, mapping_mode="clinical", output_dir=tmp_path / "b", skip_uncertainty=True
    )
...
def test_scienceplots_available_and_paper_figures_high_dpi():
    ...
    from analysis.plots import resolve_serif_font

    # Prefer Times; allow Liberation/STIX/DejaVu fallback (CI may lack TNR).
    resolved = resolve_serif_font()
    assert resolved  # non-empty family name
