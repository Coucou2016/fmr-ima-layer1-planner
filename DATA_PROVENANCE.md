...
|--------|----------|-----------------|
| Galili et al. RSOS 2022 | 10.1098/rsos.211464 | Peak-systole AP / ROA / leakage table anchors |
...
- Dryad-derived extras when archives present: `contact_fraction`, `n_sph_particles` (full-domain cloud; **not** LA↔aorta partition)
- Peak-systole AP / ROA / leakage remain table-backed when Dryad dumps lack annulus landmarks / chamber labels (honest fill notes in CSV)
- Galili peak-systole **IMA-AP 50% AP = 15.9 mm** (never undeformed 34.4)

## Download / checksums

Automated path (Anubis PoW):

```powershell
python tools/download_dryad_anubis.py
# or
python tools/import_galili_dryad.py --download --process
```

Checksums and timestamps: `data/provenance.yaml`, `data/raw/galili_dryad/MANIFEST.json`.  
Large zips are gitignored; drop them under `data/raw/galili_dryad/` locally.
...
Optional phenomenological train-fold leakage scale: `python tools/loo_evaluate.py --mode loo --refit-scale --write`
